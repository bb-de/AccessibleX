...
export function setupWidgetRoutes(app: express.Express) {
  // Analytics endpoint for widget usage tracking
  app.post('/api/widget/analytics', (req: Request, res: Response) => {
    try {
      res.setHeader('Access-Control-Allow-Origin', '*');
      res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
      res.setHeader('Access-Control-Allow-Headers', 'Content-Type');

      const { tokenId, action, data } = req.body;

      // Simple analytics logging (in production, save to database)
      ...
      res.status(200).json({ success: false }); // Return 200 to not break widget
    }
  });
  ...
  // Main widget JavaScript file serving
...
(function() {
  ...
  setTimeout(() => {
    if (!document.getElementById('accessibility-toggle')) {
      ...
      res.status(500).send('console.error("Error loading widget");');
    }
  });
